...
Earlier on in the BioDT project, rudimentary versions of the RP and biodiversity components were combined in a single raster overlay. However, this was limited to pre-calculated 'static' raster layers covering a small region (the Cairngorms National Park) rather than all of Scotland.
...
Test footnote^[This is a test footnote.].
...
$$
f_{ij}(x) = \begin{cases}
1 & \text{if feature present in pixel} \\
0 & \text{if feature absent in pixel} 
\end{cases}
$$
...
0 & \text{if feature} \gtrapprox 500\text{m away} 
\end{cases}
$$
...
$$
C_i(x, y) = \mathrm{normalise}\left\{ \sum_{j=1}^{N_\text{features}} \rho_{ij} \, f_{ij}(x, y)\right\}
$$



$$
\mathrm{RP}(x,y) = \mathrm{normalise} \left\{ \sum_{i=1}^4 C_i(x,y) \right\}
$$
