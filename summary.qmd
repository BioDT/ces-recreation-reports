...
There is another essential set of inputs to the model which are not specified by the user. These are a set of 87 _feature layers_ --- one for each of the 87 items --- covering Scotland at a resolution of 20x20m.

:::{.callout-note}
#### Items and feature layers
We use the term _items_ in the context of a persona (i.e. a single persona score is an _item_), and _feature layers_ in the context of raster data (a single layer of a raster is a _feature_).

There is a one-to-one relationship between the 87 items and the 87 feature layers.

Thus, they are described by the same set of labels. For example, the user must provide a score for the `Water_Lakes_1` (Pond) _item_, and the input dataset also contains a _feature layer_ called `Water_Lakes_1`.
:::
...
The strongest preferences of the *Soft Recreationalist* persona are highlighted in @tbl-sft.
...
The outputs of the RP model for the *Hard Recreationalist* and *Soft Recreationalist* personas for the whole of Scotland will soon be available for download as a data product [@Tigli2025]. The results are previewed in @fig-scotlandRP.
