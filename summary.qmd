...
The interactive demo is only available in the HTML version of this report. This is available online at <https://biodt.github.io/ces-recreation-reports>.
:::
...
Here an interactive demo for the model ran using the hard recreationalist preset persona for the Bush Estate area is showed. First the 4 components layers are showed. You can explore each of the layers (top right corner of the map). Not all layers had values for the specific Bush Estate area so some layers are not mapped. The underlying map was made black and white to make the layers with few values in it more visible. As it is possible to see the components of Landscape (SLSRA) and Natural features (FIPS_N) have either the features present or the feature absent; whilst the Infrastructure and Water components show the distance gradient.
...
The output of the RP model ran for this example case and a Hard Recreationalist persona profile (from the presets available in the package) is showed in the map below. The contribution of each component to the RP can be observed by switching layer from the controls in the top right corner of the map.
...
## Scotland wide datasets

We ran the RP model across its full domain using the tow "presets" persona of a Hard Recreationalist and a Soft Recreationalist (results showed as figure in [#fig_scotland_RP](#fig_scotland_RP)). The resulting datasets have been published to Zenodo.

![Recreational Potential (RP) for the whole of Scotland using the two persona presets: hard recreationalist and soft recreationalist.](images/RP_Scotland_figure.png){#fig_scotland_RP fig-align="center"}
