...
Initially, a total of five weeks (split 3:2 between JMR:MT) were allocated for this work. However, more time was made available as the scope of the work increased as described below.
...
The original aim of [#18](https://github.com/BioDT/uc-ces-recreation/pull/18) was to _refactor_ the existing code to make it possible to run the RP model via a script, independently of the Shiny app. As explained earlier, however, the decision was taken to rewrite the code from scratch.
...
In the process of arriving at a mathematical definition of the RP model (==TODO, link to Tech Supp==), it became transparent that the 'dynamical' part --- i.e. the calculations involving user-provided inputs --- was little more than a sum of pre-computed raster layers weighted by the persona scores. The 'non-dynamical' part dealt with quantities that only needed to be computed _once_, ahead of time, and stored for later use.
...
This decomposition into a one-time pre-processing stage and a dynamical stage, which would be rendered transparent by any good definition of the model, is the essential reason for the multiple orders of magnitude improvement in speed and memory use. 
...
There were also a number of subtle bugs in the original code that led to incorrect outputs ([#22](https://github.com/BioDT/uc-ces-recreation/issues/22), [#23](https://github.com/BioDT/uc-ces-recreation/issues/23)) and were fixed during the rewrite.
...
We also created a container for the input data production script, anticipating that this would need to be run on LUMI or wherever else we could access a high-memory compute node for the calculation of distances (see @sec-distance).
...
- Packaging in R does not feel as natural as Python
- Started with separate directories containing model, data production, and Shiny app, since these had different dependencies, and it seemed to make sense to let the Shiny app depend on the model package as a dependency.
- Decided to return to a single-package layout for simplicity and maintenance reasons; it was too awkward to develop the model code and the Shiny app code simultaneously.

- Use of `inst/`

- Included Lintr and Styler packages which help make the format consistent as well as catching syntax bugs. Maintaining a consistent format is useful for ensuring that meaningful changes in commits are not obscured by formatting changes.
- Added a couple of simple tests which run using `testthat`. However, due to the time pressures we did not develop any further tests, which is unfortunate.
- Added pre-commit hooks which run the tests, but also a script that can run them in case the developer does not want to install pre-commit.

- Finally, we added `Roxygen2`-style documentation for all the functions, which produces nicely formatted Markdown documentation in RStudio.
- We added some vignettes that demonstrate basic usage, although this could be expanded.
- Vignettes are run during the build stage of the package. This meant they could not use the full input data since (i) it is unacceptable to have to download 2GB data just to build the package, and (ii) we did not want the package to contain 2GB data itself. To get around this, we included some small rasters in the package (`inst/extdata/rasters/Bush`) and wrote some helper functions for accessing it.
...
### Proximity contributions and pair-wise distances
...
- https://github.com/BioDT/uc-ces-recreation/pull/92
- https://github.com/BioDT/uc-ces-recreation/pull/95
...
| [Explanation for 'proximity' contributions #19](https://github.com/BioDT/uc-ces-recreation/issues/19)                                                    | Documentation / Research required                                               | I.e. justify the use of Eq. @eq-proximity      |
| [Explain how contributions to the recreational potential are weighted #25](https://github.com/BioDT/uc-ces-recreation/issues/25)                         | Documentation / Research required                                               | I.e. justify the use of Eq. ==TODO==           |
...
| [Modify logistic function parameters so that proximity scores fall off over a longer distance #39](https://github.com/BioDT/uc-ces-recreation/issues/39) | Research required                                                               | I.e. justify the use of Eq. ==TODO==           |
...
| [Relationship between the Recreational Potential model/app and 'Digital Twins' #53](https://github.com/BioDT/uc-ces-recreation/issues/53)                | Documentation                                                                   | Discussed in Sec ==TODO==                      |
...
Instead, we will focus on a more

### Re-engineering the data production process



- Should reproject to a consistent grid at the earliest possible time, I think
- Skip the creation of categorical rasters altogether?
- Proximity stuff


Use descriptive names for the features / layers {#sec-layernames}

https://github.com/BioDT/uc-ces-recreation/issues/49

#### Part 1

Here is the function that recasts a single-layer categorical-valued raster to a `N_categories`-layered boolean-valued raster.
...
one_hot_layer <- function(infile, outfile, feature_mapping) {
    ...
    names(layer) <- feature_mapping  # This is the problem
}
```

I was very happy to find that `lapp` lets you write chunks directly to the filesystem, rather than trying to keep them in memory. However, I still needed to apply the layer names to the saved raster.

Chat-GPT has been a fairly good friend to me for learning R over the last few weeks, but in this case it confidently assured me that setting the names of `layer` would set the names of the saved raster. I really should have assumed that this would not work, but I still find R [quite spooky](https://stackoverflow.com/questions/70773297/what-is-non-standard-evaluation-and-how-can-you-pass-an-undefined-variable-to) and `terra` is all about not loading your data until absolutely necessary, so I gave it a go.

In fact it did not write these layer names - the layers took on their default names `lyr1, lyr2, ...`, but I did not check this until now.

#### Part 2

One might assume that missing layer names would produce a bug somewhere down the line. Unfortunately this did not happen.

In the next step, the one-hot encoded rasters are stacked into a single raster per component using the following function:
...
Now, it turns out that the default behaviour of `terra::rast` is to use existing layer names _if they are provided_, and otherwise _assign layer names based on the file name_.

So, because the layer names were not set in the one-hot encoding step, the layer names are _reassigned_ during this stacking step, _based on the order in which those layers appear in the raster._

As a specific example, the `FIPS_I_RoadsTracks` features are _reassigned_ the names `FIPS_I_RoadsTracks_1`, `FIPS_I_RoadsTracks_2`, and so on.

So now we see why this did not caused anything to crash down the line - these are precisely the same layer names that I _thought_ I was setting in the one-hot encoding step, but the numbers 1, 2, 3, ... are actually the order in which these layers appear in the stack, _not_ their layer name as defined in `config.csv`. I.e. **these are the correct layer names, but in the wrong order!**

#### Fix

So, how _should_ I have changed the layer names after one-hot-encoding?

I want to keep writing chunks straight to disk in the `lapp` step, obviously.

I had hoped that the solution would be to do something like

```r
    # Note that layer is essentially set to rast(outfile)
    layer <- terra::lapp(
        layer,
        fun = one_hot_pixel,
        filename = outfile,
    ...
    writeRaster(layer, outfile, overwrite = TRUE)  # This throws an error
```

but this results in an error

```
[writeRaster] source and target filename cannot be the same
```

The reason is that `terra` refuses to write to the same file that it is reading from, which is quite reasonable except that _all I want to write is some metadata_.

I have not so far been able to find a way to modify metadata without creating a copy of the data, which is very annoying. Ideally I would set the name inside `one_hot_pixel` but this is not possible.

The solution is to use the `wopt` argument to `lapp`, which is a list of args to pass to `writeRaster`, and includes the `names` argument.
...
```r
    layer <- terra::lapp(
        ...
        filename = outfile,
        overwrite = TRUE,
        wopt = list(names = feature_mapping)
    )
```

Horay.

Essentially `terra::rast(list_of_rasters)` does not seem to preserve the layer names, and once again generated default layer names that were (heavy sigh) the same as the names we have been using, but in a different order..

#### Conclusion / lessons

Now I understand this behaviour of `terra`, I can see that it was a terrible idea to use these layer names. It would have been better to have used the `Description` as the layer name instead.

#### Reducing the size of input rasters

The four input rasters are currently between 100MB and 1GB each. This is probably fine, but it's worth making a note of some possible ways to reduce their size in case any of the following are desired in future:

- Scaling up to a larger area than Scotland
- Better support for users running the app/model locally (4GB might be a lot for some users)
- Distributing a containerised application containing the input data
- Storing multiple sets of input data (e.g. one dataset per year)

The presence/absence rasters (SLSRA, FIPS_N) need only a single bit per pixel (0 or 1). Currently these are being written as unsigned 1 byte (8-bit) integers (with the option `datatype=INT1U` in `writeRaster`) since this is the smallest available according to the [documentation](https://rspatial.github.io/terra/reference/writeRaster.html), although [this site](https://rspatial.org/raster/pkg/appendix2.html) seems to imply `LOG1S` should be an option.. The `INT1U` format contains 7x more bits than needed but I'm not sure it massively matters. It could however be worth playing with the compression algorithm - it may be that certain algorithms are particularly good for sparse integer rasters (mostly zeros).

The distance-modified input rasters (FIPS_I, Water) are currently signed floats `datatype=FLT4S`. Again we could play with the compression algorithm, but there are two other options that might be fruitful.

1. **Discretise** - there is no real point in differentiating 0.5243 and 0.5244 here. Frankly it is plenty good enough to just round to the nearest 0.1. In practice we would just multiply by 10, round to the nearest integer, and store the result as an `INT1U`.
    
2. **Truncate** - [#92](https://github.com/BioDT/uc-ces-recreation/pull/92) demonstrated that the file size is massively reduced by setting pixels for which the distance to the nearest feature is >500m to `NA`, although 500m is still probably more than we need. This should be automatically satisfied by 1. since anything <0.5 will be rounded to 0.
...
On services such as DataLabs this is not permitted...
...
However, meeting this definition of Digital Twin cannot be an objective in and of itself; it must be an outcome that is realised in pursuit of other objectives. The availability of near real-time data does not justify real-time updates to the RP model. Rather, the frequency with which to update these datasets should be determined by trading off the needs of users with the monetary and environmental cost of the data acquisition and processing pipeline.
...
In the author's opinion, one of the key contributions from this period of development work was to write down a concrete, unambiguous definition of the RP model (==Link==). From this flowed the substantial improvements to the design and efficiency of the code, and the ability to verify that it is working as expected.
...
On a personal note, this was a valuable opportunity to learn to code in `R`.
