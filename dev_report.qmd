...
The data production process is not engineered for robustness..

1. Data should be reprojected onto a consistent grid at the earliest possible opportunity.
2. The projection CRS should be the same as for Leaflet maps, since that is where all raster data is eventually destined. This would avoid additional coordinate transformations during rendering and prevent the smearing of linear features and edges as described in [#61](https://github.com/BioDT/uc-ces-recreation/issues/61).
3. Layers containing linear features such as roads and rivers should not be rasterised until the last step, if at all. If nothing else, the proximity step would benefit from these layers remaining as vectors.
4. Layers should be constructed as binary presence/absence rasters from the beginning; one can skip the creation of categorical rasters altogether.
5. Layer names should be descriptive.

#### Use descriptive names for the features / layers {#sec-layernames}

The last recommendation is something we are duty bound to elaborate on, since it relates to a very subtle and serious issue that we encountered. This discussion closely follows that in [#49](https://github.com/BioDT/uc-ces-recreation/issues/49) and [#50](https://github.com/BioDT/uc-ces-recreation/issues/50).
