...
In order to pre-compute the proximity layers ([#21](https://github.com/BioDT/uc-ces-recreation/issues/21) ) we needed to switch to a *one-hot* representation, where each feature type gets its own layer that is boolean valued --- the feature is either present or not.

$$
C_i \overset{\text{one hot}}{\longrightarrow} ( C_{i0}, C_{i1}, C_{i2}, C_{i3}, \ldots)
$$
...
This decomposition into a one-time pre-processing stage and a dynamical stage, which would be rendered transparent by any good definition of the model, is the essential reason for the multiple orders of magnitude improvement in speed and memory use. Further gains resulted from a number of factors made possible through the rewrite:
...
The outputs of `terra::distance` (distances in metres) are passed through a logistic function to map them to the unit interval. There is nothing particularly important about computing distances accurately; it just matters that the outputs of the composition of the distance and logistic function fall monotically from 1 to 0 over the right range. It seemed reasonable, therefore, to swap this pair of functions for a single convolution, e.g. with a Gaussian kernel. This would reduce memory requirements for the same reason as computing the buffer using a sliding window reduces memory requirements; only small chunks of the raster need to be loaded into memory at any given time.
...
3. Replace the logistic function in the proximity part with a simple linear decrease over a fixed decrease. That is, a straight line from $(0, 1)$ to $(d_{\text{max}}, 0)$ where $d_{\text{max}}$ is a distance explicitly chosen, perhaps even by the user as a 'hyper-parameter' of the model. It would be a simple extension to make $d_\text{max}$ specific to individual items, e.g. larger for major lochs than for cycle paths.
...
The RP app is currently missing some important information that users would need to plan their activity. This includes things like parking spaces, toilets, places to eat etc.
