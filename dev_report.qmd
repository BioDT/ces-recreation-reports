...
Initially a few weeks were allocated for this work, and the two of us worked independently on small, self-contained tasks. MT worked on removing dependence on the legacy `raster` package and replacing it with a dependence on the [`terra`](https://rspatial.github.io/terra/) package, which is significantly faster, has better memory management, and supports lazy loading of large rasters. This led to tangible improvements in speed. JMR worked on [#1](https://github.com/BioDT/uc-ces-recreation/pull/1) which turned out to be a red herring.
...
The first task was therefore to move development off of DataLabs and use GitHub for version control in a multi-developer setup. The most recent (as of 22/11/2024) version of the code and data was downloaded from DataLabs and added *without modification* to a new 'orphan' git branch --- see [commit a795456](https://github.com/BioDT/uc-ces-recreation2/tree/a795456f38d9c9c26eb75418823adabaa67ac228).
...
[^1]: The `2024-model` tag on GitHub is a snapshot of all of the code that was on DataLabs. This included multiple versions of the RP model. The 'working version' at that time was kept under the [`R_ShinyVersion_ReducedModel/` subdirectory](https://github.com/BioDT/uc-ces-recreation/tree/2024-model/R_ShinyVersion_ReducedModel).
...
-   [A working Singularity container with minimal modifications to the existing code #1](https://github.com/BioDT/uc-ces-recreation/pull/1)
...
The original aim of [#18](https://github.com/BioDT/uc-ces-recreation/pull/18) was to *refactor* the existing code to make it possible to run the RP model via a script, independently of the Shiny app. As explained earlier, however, the decision was taken to rewrite the code from scratch. A number of subtle bugs in the original code that led to incorrect outputs ([#22](https://github.com/BioDT/uc-ces-recreation/issues/22), [#23](https://github.com/BioDT/uc-ces-recreation/issues/23)) were also fixed during the rewrite.

In the process of arriving at a mathematical definition of the RP model ([Section 1, Technical Supplement](tech_supp.html#sec-techdesc)), it became transparent that the 'dynamic' part --- i.e. the calculations involving user-provided inputs --- could be separated from the 'non-dynamical' part, which dealt with quantities that only needed to be computed *once*, ahead of time, and stored for later use.
...
The main substantive task was to configure parsing command line inputs. We decided against using third-party packages ([optparse](https://github.com/trevorld/r-optparse/), [argparse](https://github.com/trevorld/r-argparse)) and stick with base R functionality to reduce the future maintenance burden. The result is a very simple command-line interface, lacking nice-to-have features such as argument validation, but sufficient for a 'minimum working example' to be handed over to the BioDT team.
...
[^2]: In truth, while this was the intention, it was not quite what was being calculated - see [#22](https://github.com/BioDT/uc-ces-recreation/issues/22))
...
In the case of linear features such as roads, paths and rivers, it would have been *significantly* more efficient to compute a buffer region using the original vector layers ([#51](https://github.com/BioDT/uc-ces-recreation/issues/51)). Unfortunately we were not able to obtain access to these layers at the time.
...
| -------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------- | ------------------------------------------------------ |
| [Use Google Sheets for persona configuration #11](https://github.com/BioDT/uc-ces-recreation/issues/11)                                                  | User experience                                                                 | See @sec-storage                                       |
...
| [Modify logistic function parameters so that proximity scores fall off over a longer distance #39](https://github.com/BioDT/uc-ces-recreation/issues/39) | Changes to the model                                                            | Research required                                      |
| [Use descriptive feature/layer names #50](https://github.com/BioDT/uc-ces-recreation/issues/50)                                                          | Good practice                                                                   | **Important** (see @sec-layernames)                    |
...
| [Download map from shiny app #59](https://github.com/BioDT/uc-ces-recreation/issues/59)                                                                  | User experience                                                                 |                                                        |
| [Project rasters onto ESPG:3857 ahead of time #61](https://github.com/BioDT/uc-ces-recreation/issues/61)                                                 | Efficiency                                                                      | **Important**                                          |
| [Remove requirement for filesystem write access #71](https://github.com/BioDT/uc-ces-recreation/issues/71)                                               | App deployment                                                                  | Required for deployment on DataLabs (see @sec-storage) |
...
2.  **Do not rescale the final result either**, or at least make it a *global* rescaling by dividing by the maximum value of RP possible, not just the maximum value in the chosen region. Note that the colour mapping can still be adjusted to make sure different values are clearly differentiable.
...
Finally, removing the rescaling step takes the possibility of encountering a divide-by-zero off the table. Recall from the [Technical Supplement](tech_supp.html#sec-rescalings) that this occurs when the maximum and minimum values for a component in the region of interest are equal. This is not unlikely to occur if the user selects a small region of interest, or decides to score an entire component with the same values --- e.g. zero if the user intends to nullify contributions from a component. Currently the model does not crash in these situations since we defined some safe handling of `NA` values, but it is not ideal.
...
#### Storage of personas {#sec-storage}
...
In the author's opinion, one of the key contributions from this period of development work was to write down a concrete definition of the RP model ([Technical Supplement, Section 1](tech_supp.html#sec-techdesc)). From this flowed the substantial improvements to the design and efficiency of the code, and the ability to verify that it is working as expected.
